...
SELECT custid,
  STRING_AGG(CAST(orderid AS VARCHAR(10)), ',')
    WITHIN GROUP(ORDER BY orderid DESC) AS custorders
FROM Sales.Orders
GROUP BY custid;
...
SELECT orderid, custid, empid, orderdate
FROM Sales.Orders
WHERE orderdate = '20190212';

SELECT orderid, custid, empid, orderdate
FROM Sales.Orders
WHERE orderdate = CAST('20190212' AS DATE);

-- Language dependent
SET LANGUAGE British;
SELECT CAST('02/12/2019' AS DATE);
...
-- Language neutral
SET LANGUAGE British;
SELECT CAST('20190212' AS DATE);

SET LANGUAGE us_english;
SELECT CAST('20190212' AS DATE);

SELECT CONVERT(DATE, '02/12/2019', 101);

SELECT CONVERT(DATE, '02/12/2019', 103);

SELECT PARSE('02/12/2019' AS DATE USING 'en-US');

SELECT PARSE('02/12/2019' AS DATE USING 'en-GB');
...
SELECT orderid, custid, empid, orderdate
FROM Sales.Orders
WHERE orderdate >= '20190201' AND orderdate < '20190301';
...
SELECT PARSE('02/12/2019' AS DATETIME USING 'en-US');
SELECT PARSE('02/12/2019' AS DATETIME USING 'en-GB');
...
SELECT
  CAST('20190212 12:00:00.0000000' AS DATETIME2)
    AT TIME ZONE 'Pacific Standard Time' AS val1,
  CAST('20190812 12:00:00.0000000' AS DATETIME2)
    AT TIME ZONE 'Pacific Standard Time' AS val2;

-- Converting datetimeoffset values
-- behavior similar to SWITCHOFFSET
SELECT
  CAST('20190212 12:00:00.0000000 -05:00' AS DATETIMEOFFSET)
    AT TIME ZONE 'Pacific Standard Time' AS val1,
  CAST('20190812 12:00:00.0000000 -04:00' AS DATETIMEOFFSET)
    AT TIME ZONE 'Pacific Standard Time' AS val2;

-- DATEADD
SELECT DATEADD(year, 1, '20190212');

-- DATEDIFF
SELECT DATEDIFF(day, '20180212', '20190212');

SELECT DATEDIFF_BIG(millisecond, '00010101', '20190212');
...
SELECT DATEPART(month, '20190212');

-- DAY, MONTH, YEAR

SELECT
  DAY('20190212') AS theday,
  MONTH('20190212') AS themonth,
  YEAR('20190212') AS theyear;

-- DATENAME
SELECT DATENAME(month, '20190212');

SELECT DATENAME(year, '20190212');

-- ISDATE
SELECT ISDATE('20190212');
SELECT ISDATE('20190230');

-- fromparts
SELECT
  DATEFROMPARTS(2019, 02, 12),
  DATETIME2FROMPARTS(2019, 02, 12, 13, 30, 5, 1, 7),
  DATETIMEFROMPARTS(2019, 02, 12, 13, 30, 5, 997),
  DATETIMEOFFSETFROMPARTS(2019, 02, 12, 13, 30, 5, 1, -8, 0, 7),
  SMALLDATETIMEFROMPARTS(2019, 02, 12, 13, 30),
...
SELECT
  DATABASEPROPERTYEX(N'TSQLV5', 'Collation');
